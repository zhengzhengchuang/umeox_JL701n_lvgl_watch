...
#include "app_main.h"
#include "vm.h"
#if 0//TCFG_BROADCAST_ENABLE
#include "broadcast_api.h"
#include "le_audio/broadcast_audio_sync.h"
#endif
...
static struct broadcast_ops_callback broadcast_ops_cb = {
...
void music_player_destroy(void)
{
    if (__this) {
        ...
        if (broadcast_ops_cb.sync_hdl) {
            app_broadcast_sync_close(&broadcast_ops_cb.sync_hdl);
        }
        if (broadcast_ops_cb.dec_hdl) {
            app_broadcast_dec_close(&broadcast_ops_cb.dec_hdl);
        }
#endif
        free(__this);
        __this = NULL;
...
int music_player_decode_start(FILE *file, struct audio_dec_breakpoint *dbp)
{
...
#if 0//TCFG_BROADCAST_ENABLE
    u32 sync_output_sr = 48000;
    struct app_broadcast_sync_var sync_var = {
        .source = BROADCAST_SOURCE_FILE_BLOCK,
        .input_sample_rate = sync_output_sr,
        .output_sample_rate = sync_output_sr,
        .nch = 2,
        .delay_time = 45,
    };
    if (!broadcast_ops_cb.sync_hdl) {
        app_broadcast_sync_open(&sync_var, &broadcast_ops_cb.sync_hdl);
    }

    struct app_broadcast_dec_var dec_var = {
        .sync_hdl = broadcast_ops_cb.sync_hdl,
        .nch = 2,
        .coding_type = AUDIO_CODING_PCM,
        .sample_rate = sync_output_sr,
        .frame_size = 100,
        .bit_rate = 64000,
    };

    if (get_broadcast_role()) {
        dec_var.coding_type = AUDIO_CODING_LC3;
    } else {
        dec_var.coding_type = AUDIO_CODING_PCM;
    }

    if (!broadcast_ops_cb.dec_hdl) {
        app_broadcast_dec_open(&dec_var, &broadcast_ops_cb.dec_hdl, broadcast_ops_cb.bis_hdl);
    }
    ...
    struct file_dec_params file_params = {
        ...
        .format_priv = broadcast_ops_cb.sync_hdl->bcsync,
...
{
    broadcast_get_hdl_register(&broadcast_ops_cb);
}
