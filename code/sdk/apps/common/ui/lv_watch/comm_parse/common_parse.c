...
static bool umeox_common_le_verify(uint8_t *rev_buf, \
    uint16_t rev_len)
{ 
    uint32_t verify_sum = 0;
    uint8_t verify_sum_low8bit = 0;

    for(uint8_t i = 0; i < rev_len - 1; i++)
        verify_sum += rev_buf[i];

    verify_sum_low8bit = (uint8_t)(verify_sum&(0xff));

    if(verify_sum_low8bit == rev_buf[rev_len - 1])
        return true;
...
void umeox_common_le_cmd_parse(uint8_t *rev_buf, \
    uint16_t rev_len)
{
    uint8_t rev_cmd = rev_buf[0];

    if(!umeox_common_le_verify(rev_buf, rev_len))
    {
    ...
    switch(rev_cmd)
    {
        ...
        case le_cmd_set_dev_time:
            common_le_set_watch_time(&rev_buf[1], rev_len - 2);
            break;
