...
comm_time_para_t comm_time_para = {0};

/*********************************************************************************
                                  时间参数                                       
*********************************************************************************/
static uint8_t time_group_num = 0;
static comm_time_para_t comm_time_para_cache[Time_Group_Max] = {0};
static comm_time_para_t *p_comm_time_para[Time_Group_Max] = {NULL};
static uint16_t time_dsc_idx[Time_Group_Max*Time_Obj_Max] = {0xffff};
static lv_obj_t *common_widget_time[Time_Group_Max*Time_Obj_Max] = {NULL};
...
void common_time_widget_init(void)
{
    ...
    memset(common_widget_time, 0, \
        sizeof(common_widget_time));
...
void common_time_refresh(void)
{
    ...
    if(time_format == comm_time_format_12)
    {
        if(time_hour > 12)
            time_hour -= 12;
        else if(time_hour == 0)
    ...
    for(uint8_t i = 0; i < time_group_num; i++)
    {
        uint32_t num_addr_index = \
            p_comm_time_para[i]->num_addr_index;
        ...
        for(uint8_t j = 0; j < Time_Obj_Max; j++)
        {
            ...
            if(!common_widget_time[j + i*Time_Obj_Max] || \
                time_dsc_idx[j + i*Time_Obj_Max] == 0xffff)
                continue;

            common_widget_img_replace_src(common_widget_time[j + i*Time_Obj_Max], \
    ...
    comm_time_mode_t time_mode)
{
    ...
    uint32_t addr_index[Time_Obj_Max];
    ...
    memcpy(&comm_time_para_cache[time_group_num], \
        time_para, sizeof(comm_time_para_t));

    p_comm_time_para[time_group_num] = \
        &comm_time_para_cache[time_group_num];
    ...
    uint32_t num_addr_index = \
        p_comm_time_para[time_group_num]->num_addr_index;
    ...
    switch(time_mode)
    {
        case comm_time_mode_hh:
            obj_num = 2;
            obj_idx = 0 + \
                time_group_num*Time_Obj_Max;
            break;

        case comm_time_mode_mm:
            obj_num = 2;
            obj_idx = 3 + \
                time_group_num*Time_Obj_Max;
            break;

        case comm_time_mode_ss:
            obj_num = 2;
            obj_idx = 6 + \
                time_group_num*Time_Obj_Max;
            break;

        case comm_time_mode_hh_mm:
            obj_num = 5;
            obj_idx = 0 + \
                time_group_num*Time_Obj_Max;
            break;

        case comm_time_mode_mm_ss:
            obj_num = 5;
            obj_idx = 3 + \
                time_group_num*Time_Obj_Max;
            break;

        case comm_time_mode_hh_mm_ss:
    ...
    widget_img_para.img_parent = \
        p_comm_time_para[time_group_num]->time_parent;
    ...
    for(uint8_t i = 0; i < obj_num; i++)
    {
        ...
        widget_img_para.img_x = \
            p_comm_time_para[time_group_num]->time_x + \
                width_sum + p_comm_time_para[time_group_num]->num_inv*i;
        widget_img_para.img_y = \
            p_comm_time_para[time_group_num]->time_y;
        widget_img_para.file_img_dat = \
            time_addr_index[obj_idx%Time_Obj_Max + i];
        common_widget_time[obj_idx + i] = \
