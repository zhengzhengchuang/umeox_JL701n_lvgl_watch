...
comm_date_para_t comm_date_para = {0};

/*********************************************************************************
                                  日期参数                                       
*********************************************************************************/
static uint8_t date_group_num = 0;
static comm_date_para_t *p_comm_date_para[Date_Group_Max] = {NULL};
static comm_date_para_t comm_date_para_cache[Date_Group_Max] = {0};
static uint16_t date_dsc_idx[Date_Group_Max*Date_Obj_Max] = {0xffff};
static lv_obj_t *common_widget_date[Date_Group_Max*Date_Obj_Max] = {NULL};
...
void common_date_widget_init(void)
{
    ...
    memset(common_widget_date, 0, \
        sizeof(common_widget_date));
...
void common_date_refresh(void)
{
    ...
    for(uint8_t i = 0; i < date_group_num; i++)
    {
        uint32_t num_addr_index = \
            p_comm_date_para[i]->num_addr_index;
        ...
        for(uint8_t j = 0; j < Date_Obj_Max; j++)
        {
            ...
            if(!common_widget_date[j + i*Date_Obj_Max] || \
                date_dsc_idx[j + i*Date_Obj_Max] == 0xffff)
                continue;

            common_widget_img_replace_src(common_widget_date[j + i*Date_Obj_Max], \
    ...
    comm_date_mode_t date_mode)
{
    ...
    memcpy(&comm_date_para_cache[date_group_num], \
        date_para, sizeof(comm_date_para_t));

    p_comm_date_para[date_group_num] = \
        &comm_date_para_cache[date_group_num];
    ...
    uint32_t num_addr_index = \
        p_comm_date_para[date_group_num]->num_addr_index;
    ...
    switch(date_mode)
    {
        case comm_date_mode_yyyy:
            obj_num = 4;
            obj_idx = 0 + \
                date_group_num*Date_Obj_Max;
            break;

        case comm_date_mode_mm:
            obj_num = 2;
            obj_idx = 5 + \
                date_group_num*Date_Obj_Max;
            break;

        case comm_date_mode_dd:
            obj_num = 2;
            obj_idx = 8 + \
                date_group_num*Date_Obj_Max;
            break;

        case comm_date_mode_yyyy_mm:
            obj_num = 7;
            obj_idx = 0 + \
                date_group_num*Date_Obj_Max;
            break;

        case comm_date_mode_mm_dd:
            obj_num = 5;
            obj_idx = 5 + \
                date_group_num*Date_Obj_Max;
            break;

        case comm_date_mode_yyyy_mm_dd:
    ...
    widget_img_para.img_parent = \
        p_comm_date_para[date_group_num]->date_parent;
    ...
    for(uint8_t i = 0; i < obj_num; i++)
    {
        ...
        widget_img_para.img_x = \
            p_comm_date_para[date_group_num]->date_x + \
                width_sum + p_comm_date_para[date_group_num]->num_inv*i;
        widget_img_para.img_y = \
            p_comm_date_para[date_group_num]->date_y;
        widget_img_para.file_img_dat = \
            date_addr_index[obj_idx%Date_Obj_Max + i];
        common_widget_date[obj_idx + i] = \
