...
static void title_label_create(menu_align_t menu_align)
{
    ...
    widget_label_para.label_text = \
        get_lang_txt_with_id(lang_txtid_azkar);
...
static void elem_container_cb(lv_event_t *e)
{
    ...
    if(language == comm_language_id_arabic)
        SetAzkarTxtLang(AzkarTxtLangAr);
    else
        SetAzkarTxtLang(AzkarTxtLangEn);
    set_azkar_sec_scroll_offset(0);

    ui_menu_jump(ui_act_id_azkar_section);
...
#if 0
    memset(&azkar_list_ctx, 0, \
        sizeof(azkar_list_ctx_t));

    azkar_list_title_container_create(obj);

    azkar_list_container_create(obj);

    azkar_list_layout_create();
#endif
