...
static void translate_menu_released_handle(void)
{
    ...
    if(*translate_scroll_dir == LV_DIR_HOR)
    {
        ...
        if(LV_ABS(translate_scroll_sum->x) < lcd_w/4)
    ...
    }else if(*translate_scroll_dir == LV_DIR_VER)
    {
        ...
        if(LV_ABS(translate_scroll_sum->y) < lcd_h/4)
            widget_anim_para.anim_end_val = 0;
        else
        {
            if(translate_scroll_sum->y > 0)
    ...
    anim_duration = anim_duration > 400?400:anim_duration;
...
void translate_menu_create(lv_obj_t *obj)
{
    ...
    for(uint8_t i = 0; i < valid_cnt; i++)
    {
        ...
        tileview_dir_point.x = p_ui_info_cache->ui_translate_info.translate_dir_points[i].x;
        tileview_dir_point.y = p_ui_info_cache->ui_translate_info.translate_dir_points[i].y;

        widget_obj_para.obj_x = tileview_dir_point.x*lcd_w;
        widget_obj_para.obj_y = tileview_dir_point.y*lcd_h;
        ...
        uint8_t idx = translate_menu_cache_by_point(&tileview_dir_point);
        if(idx != 0xff)
    ...
    lv_obj_add_event_cb(obj, translate_menu_event_cb, LV_EVENT_ALL, NULL);
