...
//             my_px_num += lv_area_get_size(&disp_refr->inv_areas[i]);
//         }
//     }
...
// #endif
    ...
    if(usr_full_screen){
        full_screen_flush();
        // lv_obj_invalidate(lv_scr_act());//触发全屏重绘    //双PSRAM BUFFER要打开    
    }

#endif  
    int b = jiffies_msec();
    static int last = 0;
    // printf("full  psram %d  elaps:%d b_fps:%d t_fps:%d px_num:%d ", b-a, a-c, 1000/((b-a+a-c)?(b-c):1), 1000/((b-last)?(b-last):1), px_num);
    last = b;
}
