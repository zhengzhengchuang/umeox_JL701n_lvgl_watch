...
*/
/*----------------------------------------------------------------------------*/
static u16 check_head(RECFILESYSTEM *pfs, RECFILEHEAD *pfile_h, u16 sector)
{
    //RECFILEHEAD file_h;
    u32 addr;
    u32 total_len;
...
RECFILESYSTEM *recfs = NULL;
REC_FILE *recfile = NULL;
#if 1
...
static int nor_fs_mount(struct imount *mt, int cache_num)
{
    ...
    fd = mt->dev.fd;
    recfs = (RECFILESYSTEM *)zalloc(sizeof(RECFILESYSTEM));
    ASSERT(recfs);
    recfile = (REC_FILE *)zalloc(sizeof(REC_FILE));
    ASSERT(recfile);
    /* hook_norflash_open(); */
...
{
    y_printf(">>>[test]:goto nor_fs_unmount\n");
    if (recfs) {
        free(recfs);
        recfs = NULL;
    }
