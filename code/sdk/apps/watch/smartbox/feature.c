...
#endif
    // 告诉app当前升级前检查电量的阈值；防止low_battery_level设置大于10
    if (smart->low_battery_level % 10) {
        sys_info.low_battery_level = (smart->low_battery_level % 10 + 1) * 10;
    }
...
static u32 target_feature_md5_game_support(void *priv, u8 attr, u8 *buf, u16 buf_size, u32 offset)
{
    ...
    u8 dat[2] = {0};
    dat[0] = md5_support;
    dat[1] |= BIT(2); // 获取设备配置信息
    rlen = add_one_attr(buf, buf_size, offset,  attr, dat, sizeof(dat));
#endif
