...
static void smartbox_common_event_deal(int msg, int argc, int *argv)
{
    ...
    case USER_MSG_SMARTBOX_SPORT_DATA_EVENT:
        ...
        break;
    case USER_MSG_SMARTBOX_GET_SYS_INFO_OVERLOAD_OPT:
        printf("USER_MSG_SMARTBOX_GET_SYS_INFO_OVERLOAD_OPT\n");
        // 参数 ：类型 + 自定义参数
        if (1 == argv[0]) { // 闹钟处理
#if (TCFG_RTC_ENABLE && !SMARTBOX_APP_RTC_EN)
            extern u32 rtc_info_notify_by_mass_data(u32 mask);
            rtc_info_notify_by_mass_data((u32)argv[1]);
#endif
        }
        break;
    default:
