...
static void file_transfer_verify_heartbeat_packet(void)
{
    ...
    if (ftp_d) {
        ...
        if (heartbeat_cnt != cur_cnt) {
            ...
            JL_CMD_send(JL_OPCODE_FILE_TRANSFER, ftp_d->win, 7, 0);
...
static void __file_transfer_download_file_check_caculate(void *priv)
{
    ...
    for (int i = 0; (ftp_d->file && (i < cnt)); i++) {
    ...
    if (ftp_d->check.counter == 0) {
        if (ftp_d->file && ftp_d->check.remain) {
        ...
        //crc_check end
        if (ftp_d->file && ftp_d->check.crc_tmp == ftp_d->file_crc) {
...
static int file_transfer_watch_opt(u8 flag, u8 OpCode_SN)
{
    int ret = 0;
    // 假如当前是大文件传输表盘：
    if (BS_FLASH == ftp_d->dev_handle) { // 外挂flash存放非表盘文件，则和存放sd卡操作一致
        // 获取当前root_path
        char *root_path = NULL;
        if (ftp_d->dev) {
...
void file_transfer_download_start(void *priv, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (ftp_d->file_size > sizeof(ftp_d->win)) {
        ...
        if (ftp_d->file_offset == 0 || ftp_d->file_offset > file_size) {
            ...
            //上次文件已经收完, 进入校验流程
            //只剩最开始512数据没有写入的数据情况才会进到这里，所以需要当作最后一包处理，不然直接校验会校验不通过
