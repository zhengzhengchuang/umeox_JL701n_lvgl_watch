...
#if (defined(TCFG_EX_FLASH_POWER_IO) && (TCFG_EX_FLASH_POWER_IO != NO_CONFIG_PORT))
    soff_gpio_protect(TCFG_EX_FLASH_POWER_IO);
#endif
    /* soff_gpio_protect(SPI0_PWR_C); */
    soff_gpio_protect(SPI0_CS_C);
    soff_gpio_protect(SPI0_CLK_C);
    soff_gpio_protect(SPI0_DO_D0_C);
    soff_gpio_protect(SPI0_DI_D1_C);
    if (get_sfc1_bit_mode() == 4) {
        soff_gpio_protect(SPI0_WP_D2_C);
        soff_gpio_protect(SPI0_HOLD_D3_C);
    }
#endif
