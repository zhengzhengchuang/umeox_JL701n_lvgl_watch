...
static void touchpad_get_xy(lv_coord_t *x, lv_coord_t *y)
{
    ...
    int16_t temp_x = get_dev_touch_x();
    int16_t temp_y = get_dev_touch_y();

    if(temp_x < 0)
        (*x) = 0;
    else if(temp_x >= info.width)
        (*x) = info.width - 1;
    else
        (*x) = temp_x;
