...
static u8 uart_config_free(const struct uart_platform_data_t *arg, enum PFI_TABLE rx_pfun)
{
    if (!(arg->tx_pin < IO_PORT_MAX || arg->rx_pin < IO_PORT_MAX)) {
        return -1;
    }
...
u32 uart_dev_close(const struct uart_platform_data_t *arg, uart_bus_t *ut)
{
    ...
    if (&uart0 == ut) {
        UT0_close();
        uart_config_free(arg, PFI_UART0_RX);
        return gpio_close_uart0();
    } else if (&uart1 == ut) {
        UT1_close();
        uart_config_free(arg, PFI_UART1_RX);
        return gpio_close_uart1();
    } else {
        UT2_close();
        uart_config_free(arg, PFI_UART2_RX);
