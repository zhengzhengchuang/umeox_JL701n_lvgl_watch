...
#pragma str_literal_override(".mcpwm.text.const")
...
void mcpwm_init(struct pwm_platform_data *arg)
{
    ...
    if (arg->h_pin < IO_MAX_NUM) {      //任意引脚
        ...
        gpio_och_disable_output_signal(arg->h_pin, OUTPUT_CH_SIGNAL_MC_PWM0_H + 2 * arg->pwm_ch_num);
    ...
    if (arg->l_pin < IO_MAX_NUM) {      //任意引脚
        ...
        gpio_och_disable_output_signal(arg->l_pin, OUTPUT_CH_SIGNAL_MC_PWM0_L + 2 * arg->pwm_ch_num);
