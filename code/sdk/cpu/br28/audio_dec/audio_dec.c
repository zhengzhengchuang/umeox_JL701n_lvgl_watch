...
{
    u32 out_way = audio_app_stream_get_out_way();
    if (!(out_way & AUDIO_WAY_TYPE_DAC)) {
        return ;
    }
...
int audio_dec_init()
{
    ...
    if (config_mixer_en) {
        /*初始化mix_buf的长度*/
        audio_mixer_set_output_buf(&mixer, NULL, AUDIO_MIXER_LEN);
#ifdef CONFIG_MIXER_CYCLIC
#define MIXER_MIN_LEN		(128*4*2)
        // 设置mixer最小输出长度
        audio_mixer_set_min_len(&mixer, AUDIO_MIXER_LEN < (MIXER_MIN_LEN * 2) ? (AUDIO_MIXER_LEN / 2) : MIXER_MIN_LEN);
#if (SOUNDCARD_ENABLE)
        // 关闭直通输出
