...
*/
/*----------------------------------------------------------------------------*/
void broadcast_file_decoder_kick()
{
    if (!file_dec) {
        return ;
    }
    if (file_dec->dec_to_pcm) {
...
static void file_dec_to_pcm_wait_timeout(void *priv)
{
    struct audio_stream_entry *entry = (struct audio_stream_entry *)priv;
    ...
    sys_hi_timeout_del(hdl->timeout);
    hdl->timeout = 0;
                                        ...
                                        struct audio_data_frame *out)
{
    ...
    if (wlen != in->data_len) {
        hdl->timeout = sys_hi_timeout_add((void *)entry, (void (*)(void *))file_dec_to_pcm_wait_timeout, 4);
    }
    return wlen;
}
/*----------------------------------------------------------------------------*/
...
static int file_dec_start(void)
{
    ...
    //更新参数到同步
    if (dec->dec_to_pcm) {
        dec->format_sync(dec->format_priv, dec->file_dec.sample_rate, dec->file_dec.output_ch_num);
    }
__open_ok:
#if CHECK_SR_WHEN_DECODING
...
void file_dec_close(void)
{
    ...
    if (file_dec->file_dec.status) {
        ...
        if (file_dec->dec_to_pcm) {
            local_irq_disable();
            if (file_to_pcm_hdl.timeout) {
                sys_hi_timeout_del(file_to_pcm_hdl.timeout);
                file_to_pcm_hdl.timeout = 0;
            }
            local_irq_enable();
        }
