...
static bool bmp280_read_buf(u8 reg, u8 *buf, u8 len)
{
    ...
    delay(bmp280_iic_info->iic_delay);
...
bool bmp280_init(void *priv)
{
    ...
    /* log_info("%d,%d,%d;%d,%d,%d,%d,%d,%d,%d,%d,%d\n",bmp280_par.t1,bmp280_par.t2,bmp280_par.t3,bmp280_par.p1,bmp280_par.p2,bmp280_par.p3,bmp280_par.p4,bmp280_par.p5,bmp280_par.p6,bmp280_par.p7,bmp280_par.p8,bmp280_par.p9); */
    ...
    bmp_configstr.spi_en = 0;//DISABLE;
...
void bmp280_init_read_test()
{
    ...
    iic_init(bmp280_iic_info_test.iic_hdl);
    ...
    if (bmp280_init(&bmp280_iic_info_test)) {
        ...
        while (iii--) {
            ...
            log_info("bmp280 fixed Pressure %ld.%02dpa,temperature:%d.%02dc,alti:%d.%02dm\n", bmp_pressure / 100, bmp_pressure % 100, bmp_temperature / 100, bmp_temperature % 100, (s32)bmp280_altitude, (u32)(((s32)(abs(bmp280_altitude * 100))) % 100));
#else          //使用浮点补偿
