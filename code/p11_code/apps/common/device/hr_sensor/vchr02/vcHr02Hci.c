...
uint16_t vcHr02CalculateOSCFreq(vcHr02_t *pvcHr02)
{
    ...
    if (cntCalOsc >= 3) {
        ...
        /* printf("-----test-----:pvcHr02->vcSampleRate:%d\n",pvcHr02->vcSampleRate); */
        if (diffCountIsOk) {
            for (i = 0; i < 2; i++) {
                uint32_t temp_data = (uint32_t)(intDiffRtc[2 * i + 2] - intDiffRtc[2 * i]);
                if (temp_data == 0) {
                    printf("-----test-----:intDiffRtc:%d\n", temp_data);
                    temp_data = 1;
                }
                freDiv[i] = ((uint32_t)(intDiffAdj[2 * i + 2] - intDiffAdj[2 * i])) * pvcHr02->mcuOscValue / pvcHr02->vcSampleRate / (temp_data) - 1;
