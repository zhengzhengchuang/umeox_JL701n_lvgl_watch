...
static int simple_trans_read_file_by_id(u8 file_type, u16 id, u32 data_offset, u8 *data, u16 data_len)
{
    ...
    if (file_type == F_TYPE_SLEEP) {
        extern int motion_module_sleep_file_write(void);
        extern void motion_module_clean_sleep(void);
        extern int motion_module_has_sleep_data(void);

        /* if (motion_module_has_sleep_data()) { */
        if (!motion_module_sleep_file_write()) {
            motion_module_clean_sleep();
        }
        /* } */
