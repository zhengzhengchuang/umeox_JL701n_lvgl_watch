...
os.system('del /q "2.binfile\*.*"');

os.system('del /q "3.outputfile\*.*"');
...
# 定义输入文件夹和输出文件夹的路径
input_folder = "1.inputfile"
output_folder = "2.binfile"
...
clear_file_contents(f"./3.outputfile/{head_name}_addresses.txt")
clear_file_contents(f"./3.outputfile/{head_name}_index.c")
clear_file_contents(f"./3.outputfile/{head_name}.bin")
...
with open(f"./3.outputfile/{head_name}.bin", "wb") as o_file:
    o_file.write(output_bin)

struct_text += "};"

with open(f"./3.outputfile/{head_name}_index.c", "w") as struct_file:
    struct_file.write(struct_text)

define_text += f'#endif'
with open(f"./3.outputfile/{head_name}_index.h", "a") as define_file:
